...
class SIFTMatcher():
    ...
    def match(self,srcImg,testImg,direction, return_metrics=False):
        ...
        cv2.imwrite(f"results/matches/matches_{direction}_{time.time()}.jpg", img3)
        ...
        if len(good) > MIN_MATCH_COUNT:
            ...
            cv2.imwrite(f"results/matches/inliers_{direction}_{time.time()}.jpg", img4)
            ...
            if return_metrics:
                return_vals = {
                    ...
                    'match_distances': [m.distance for m in good],
                    'keypoints1': kp1,
                    'keypoints2': kp2,
                    'homography_det': np.linalg.det(M) if M is not None else 0
                }
                return return_vals
            return M
        else:
